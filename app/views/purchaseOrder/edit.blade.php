	...
	<body class="wide">
						...
						<div class="col-md-12">
							<div class="panel panel-default">
								<div class="panel-heading">
									<h3 class="panel-title">Update Purchase Order </h3>
								</div>
								<div class="panel-body">
									{{Form::model($purchaseOrder, array('route' => array('purchaseOrder.update', $purchaseOrder->id), 'method' => 'PUT', 'id' => 'purchaseOrderForm'))}}
									...
									<fieldset>
										<div class="form-group col-md-6">
											...
											<div class="input-group date pull-left">
												...
												<input type="text" id="date" name="date" class="form-control" value="{{ $purchaseOrder->date }}">
											</div>
										</div>
									</fieldset>
									<br>
									<fieldset>
										...
										<div class="col-md-2">
											<div class="form-group">
												...
												<input type="text" class="form-control" style="background:#eeeeee" value= "{{ $purchaseOrder->supplierName }}" disabled>
											</div>
										</div>
									</fieldset>
										...
										<fieldset>
    										...
    										<input type="hidden" id="numLines" value= "{{ count($purchaseOrderProducts) }}">

    										@foreach($purchaseOrderProducts as $key => $value)
	    										<div id="purchaseOrder-line-item" class="row row-eq-height">
	    											<input type="hidden" id="purchaseOrderLineId" value= "{{ $value->id }}">
    										...
    										<div id="service-line-item" class="row row-eq-height">
    											...
    											<div class="col-md-6">
    												<div class="form-group">
														...
														<div class="has-error">
														@endif
														{{ Form::textarea('details', $purchaseOrder->details, array('class' => 'form-control')) }}
    												</div>
    											</div>
										...
										<div class="padd-t-20">
											{{ Form::submit('Update', array('class' => 'btn btn-theme btn-lg btn-block', 'onclick' => 'return confirm("Are you sure you want to update this purchase order?")')) }}
		...
		<script type="text/javascript">
			...
			$('#purchaseOrderForm').submit(function(event) {
				...
				$("input[id='purchaseOrderLineId']").each(function() {
					var currentElement = $(this);
					var value = currentElement.val();

					$('#purchaseOrderForm').prepend('<input type="hidden" name="purchaseOrderLineIds[]" id="purchaseOrderLineIds[]" value="' + value + '" />');
				});
			...
			$(document).on('click', '.remove-purchaseOrder-item', function(e){
				...
				var numLines = parseInt($('#numLines').val());

				if (numLines > 1) {
					$('#numLines').val(numLines-1);
					$(this).parent().parent().remove();
				}
				else {
					alert("Could not remove.  A purchase order must have at least one product.");
				}
			});
